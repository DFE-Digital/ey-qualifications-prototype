...
router.post('/q1-post', function(request, response){
  if (request.session.data['awarding-location'] == undefined) {
    return response.redirect("/current/r16/q1-error")
  } 
  if (request.session.data['awarding-location'] == 'England') {
    return response.redirect("/current/r16/q2")
  } 
})
...
router.post('/q2-post', function(request, response){
  ...
  if (noStartedDate && noAwardedDate) {
    return response.redirect("/current/r16/q2-error-date-started-awarded");
  }
  if (noStartedDate) {
    return response.redirect("/current/r16/q2-error-date-started");
  }
  if (noAwardedDate) {
    return response.redirect("/current/r16/q2-error-date-awarded");
  }
  return response.redirect("/current/r16/q3");
})
...
router.post('/q3-post', function(request, response){
  switch(request.session.data['qualification-level']) {
    ...
    case "not-sure":
      ...
      response.redirect("/current/r16/q4");
      break;
    default:
      response.redirect("/current/r16/q3-error");
      break;
  }
})
...
router.post('/confirm-post-236', function(request, response) {
  ...
  if (request.session.data['yes-no'] == undefined) {
    return response.redirect(`/current/r16/eyq-236-confirm-error`)
  }
  var yesNoValue = request.session.data['yes-no'];
  if (yesNoValue == 'Yes') {
    return response.redirect(`/current/r16/eyq-236-checked`);
  } else {
    return response.redirect('/current/r16/search-results');
  }
})
...
router.post('/confirm-post-292', function(request, response) {
  var redirectValue = request.session.data['redirect'];
  if (request.session.data['yes-no'] == undefined) {
    return response.redirect(`/current/r16/eyq-292-confirm-error`)
  }
  var yesNoValue = request.session.data['yes-no'];
  if (yesNoValue == 'Yes') {
    return response.redirect(`/current/r16/eyq-292-requirement1`);
  } else {
    return response.redirect('/current/r16/search-results');
  }
})
...
router.post('/eyq-292-requirement1-post', function(request, response) {
  ...
  if (request.session.data['ledToQTS'] == undefined) {
    return response.redirect(`/current/r16/eyq-292-requirement1-error`)
  } else {
    return response.redirect(`/current/r16/eyq-292-requirement2`)
  }
})
...
router.post('/eyq-292-requirement2-post', function(request, response) {
  ...
  if (request.session.data['isQAAConsistent'] == undefined) {
    return response.redirect(`/current/r16/eyq-292-requirement2-error`)
  } else {
    return response.redirect(`/current/r16/eyq-292-requirement3`)
  }
})
...
router.post('/eyq-292-requirement3-post', function(request, response) {
  ...
  if (request.session.data['includesAssessedPractice'] == undefined) {
    return response.redirect(`/current/r16/eyq-292-requirement3-error`)
  } else {
    return response.redirect(`/current/r16/eyq-292-requirements-check`)
  }
})
...
router.get('/reset-filters', function(request, response) {
  ...
  response.redirect("/current/r16/q1");
})

// Search results
router.post('/check-your-answers-post', function(request, response) {
  if (request.session.data['awarding-organisation'] == 'none') return response.redirect('/current/r16/q4-error');

  response.redirect("/current/r16/check-your-answers");
});
...
router.post('/search-results-post', function(request, response) {
  ...
  response.redirect("/current/r16/search-results");
})
...
router.get('/clear-search', function(request, response) {
  ...
  response.redirect("/current/r16/search-results");  // Redirect back to the search results page
});

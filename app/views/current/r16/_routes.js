...
router.post('/confirm-post-293', function(request, response) {
  var redirectValue = request.session.data['redirect'];
  if (request.session.data['yes-no'] == undefined) {
    return response.redirect(`/current/r16/eyq-293-confirm-error`)
  }
  var yesNoValue = request.session.data['yes-no'];
  if (yesNoValue == 'Yes') {
    return response.redirect(`/current/r16/eyq-293-requirement1`);
  } else {
    return response.redirect('/current/r16/search-results');
  }
})
...
router.post('/eyq-293-requirement1-post', function(request, response) {
  ...
  if (request.session.data['ledToQTS'] == undefined) {
    return response.redirect(`/current/r16/eyq-293-requirement1-error`)
  } else {
    return response.redirect(`/current/r16/eyq-293-requirement2`)
  }
})
...
router.post('/eyq-293-requirement2-post', function(request, response) {
  ...
  if (request.session.data['isQAAConsistent'] == undefined) {
    return response.redirect(`/current/r16/eyq-293-requirement2-error`)
  } else {
    return response.redirect(`/current/r16/eyq-293-requirement3`)
  }
})
...
router.post('/eyq-293-requirement3-post', function(request, response) {
  ...
  if (request.session.data['includesAssessedPractice'] == undefined) {
    return response.redirect(`/current/r16/eyq-293-requirement3-error`)
  } else {
    return response.redirect(`/current/r16/eyq-293-requirements-check`)
  }
})
